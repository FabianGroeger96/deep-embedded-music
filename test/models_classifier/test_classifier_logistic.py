...
class TestClassifierLogistic(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        self.audio_pipeline = TripletsInputPipeline(params=self.params, dataset=self.dataset)
        self.classifier = ClassifierLogistic("LogisticClassifier", n_labels=len(self.dataset.LABELS))
    ...
    def test_build_model(self):
        self.classifier.build(self.audio_feature.shape)
        self.assertDTypeEqual(self.classifier, GRUNet)

    def test_output_tensor(self):
        model_input = tf.expand_dims(self.audio_feature, axis=0)
        prediction = self.classifier(model_input)
